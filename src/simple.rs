...
impl<D> CoverTreeNode<D> where D: PartialEq {
    ...
    fn find_nearest<'a>(&'a self, 
        ...
        let mut nearest = if (metric)(&self.data, query) < (metric)(nearest_yet, query) { 
            &self.data 
        } else {
            nearest_yet
        };
        ...
        for child in &self.children {
            if (metric)(nearest, query) > (metric)(&child.data, query)-self.max_distance {
                nearest = child.find_nearest(query, &nearest, metric);
            }
        }
    ...
    fn insert<'a>(node: &'a mut CoverTreeNode<D>, 
                  data: D, 
                  metric: Metric<D>,
                  span_factor: f64)
